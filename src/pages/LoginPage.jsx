...
const LoginPage = () => {
  ...
  return (
    <div className="flex flex-col justify-center items-center h-[80vh]">
      ...
      <form onSubmit={handleSubmit} className="fieldset bg-gray-800 border-gray-700 rounded-box w-xs border p-4">
        <legend className="fieldset-legend text-gray-200">Login</legend>

        <label className="label text-gray-300">Email</label>
        <input type="email" required name="email" onChange={handleChange} value={formData.email} className="input text-gray-300 bg-[#1d232a] border focus:border-none focus:outline-1 focus:outline-gray-400" placeholder="email" />

        <label className="label text-gray-300">Password</label>
        <div className="relative">
          <input type={showPassword ? 'text' : 'password'} required name="password" onChange={handleChange} value={formData.password} className="input text-gray-300 bg-[#1d232a] border focus:border-none focus:outline-1 focus:outline-gray-400" placeholder="password" />
          {formData.password &&
            (showPassword ?
        ...
        <Link className="text-blue-400 ml-2" to={'/forgot-password'}>Forgot password?</Link>
        {
          loading ?
            ...
            :
            <button type="submit" className="btn border-none shadow bg-blue-500/80 mt-3 rounded-md">Login</button>
        }
