        ...
        <div className="flex items-center justify-between p-2 lg:p-4 hover:bg-gray-900 transition-colors duration-150">
            ...
            <div className="flex items-center gap-2 lg:gap-4">
                {imgSrc ? (
                    ...
                    <div className={`size-10 lg:size-12 rounded-full flex items-center justify-center text-white text-xl font-medium ${getAvatarColor(name)}`}>
                        {getInitial(name)}
                    </div>
                )}

                <div className="flex flex-col cursor-default">
                    <span className="text-gray-200 font-semibold text-sm lg:text-base">{name}</span>
                    <span className="text-gray-300 text-xs lg:text-sm tracking-wide">{phone}</span>
                </div>
            </div>
